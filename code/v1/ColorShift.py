...
def shrink_fade_out(screen, speed=5):
...
def keybinds():
    global running, current_side, DISPLAY_WIDTH, DISPLAY_HEIGHT, screen, notes, game_state, start_time,logo_target_x, logo_target_y, text_surface
...
def draw_gradient_background(surface):
    for y in range(DISPLAY_HEIGHT):
        r = 255
        g = int(180 + (y / DISPLAY_HEIGHT) * 40)
        b = int(200 + (y / DISPLAY_HEIGHT) * 50)
        py.draw.line(surface, (r, g, b), (0, y), (DISPLAY_WIDTH, y))

# Diagonal light beam
class LightBeam:
    def __init__(self):
        self.length = random.randint(450, 500)
        self.width = 50
        self.x = random.randint(-DISPLAY_WIDTH // 10, DISPLAY_WIDTH // 10) * 7.5
        self.y = random.randint(-DISPLAY_HEIGHT // 10, DISPLAY_HEIGHT // 10) * 7.5
        self.speed = random.uniform(0.4, 1)
        self.alpha = random.randint(40, 60)

    def move(self):
        self.x += self.speed
        self.y += self.speed
        if self.x > DISPLAY_WIDTH or self.y > DISPLAY_HEIGHT:
            self.x = random.randint(-DISPLAY_WIDTH, 0)
            self.y = random.randint(-DISPLAY_HEIGHT, 0)

    def draw(self, surface):
        beam_surface = py.Surface((self.length, self.width), py.SRCALPHA)
        py.draw.rect(beam_surface, (255, 255, 255, self.alpha), (0, 0, self.length, self.width), border_radius=20)
        rotated_beam = py.transform.rotate(beam_surface, -45)
        surface.blit(rotated_beam, (self.x, self.y))
...
base_text_size = base_text_surface.get_size()

# MAIN GAMEPLAY LOOP
# ========================
beams = [LightBeam() for _ in range(15)]

# At the top with your other global variables, add logo position variables
...
def om_logo():
    ...
    logo_target_x = DISPLAY_WIDTH // 2  # Move to 1/4 of screen width
    logo_target_y = DISPLAY_HEIGHT // 3.5 # Keep vertical center

def intro():
    global elapsed, logo_x, logo_y, logo_target_x, logo_target_y
    ...
    if abs(dx) > 2 or abs(dy) > 2:
        logo_x += dx * 0.15 # Smooth movement - adjust the 0.1 for speed
        logo_y += dy * 0.15
    
    # Draw circle with updated coordinates
    py.draw.circle(screen, intro_colors["black"], (logo_x, logo_y), radius, 7)
    ...
    angles = [
        elapsed * orbit_speed,
        elapsed * orbit_speed + math.pi / 2,
        elapsed * orbit_speed + math.pi,
        elapsed * orbit_speed + 3 * math.pi / 2
    ]

    colors = [intro_colors["red"], intro_colors["blue"], intro_colors["green"], intro_colors["yellow"]]

    for i, angle in enumerate(angles):
        x = logo_x + orbit_radius * math.cos(angle)
        y = logo_y + orbit_radius * math.sin(angle)
        py.draw.circle(screen, colors[i], (int(x), int(y)), dot_radius)

    # Pulsing ColorShift text with updated coordinates
    cs_text = comfortaa_font.render("ColorShift", True, intro_colors["black"])
    orig_width, orig_height = cs_text.get_size()
    scale_factor = 1.0 + 0.05 * math.sin(elapsed * 10)
    width = int(orig_width * scale_factor)
    height = int(orig_height * scale_factor)
    scaled_surface = py.transform.scale(cs_text, (width, height))
    rect = scaled_surface.get_rect(center=(logo_x, logo_y))
    screen.blit(scaled_surface, rect)

def p_enter():
    global logo_target_x, logo_target_y
    # Fading and Pulsing Press To Play text
    text_surface = sub_font.render("Press Enter to Start", True, intro_colors["black"]).convert_alpha()
    fade = (math.sin(elapsed * 2) + 1) / 2 # Fade equation
    min_alpha, max_alpha = 100, 255
    current_alpha = min_alpha + fade * (max_alpha - min_alpha)

    text_surface.set_alpha(min_alpha + fade * (max_alpha - min_alpha))
    text_rect = text_surface.get_rect(center=(logo_target_x, logo_target_y + 240))  # Offset down below the main circle
...
def menu():
    ...
    if game_state == "menu":
        ...
        if fading_in:
            fade_alpha += 5  # Adjust speed here
            if fade_alpha >= target_alpha:
                fade_alpha = target_alpha
                fading_in = False  # Done fading in

            text_surface = sub_font.render("Press Enter to Start", True, intro_colors["black"]).convert_alpha()
            text_surface.set_alpha(fade_alpha)
            text_rect = text_surface.get_rect(center=(logo_target_x, logo_target_y + 240))
            screen.blit(text_surface, text_rect)
        else:
            fade_alpha = p_enter()  # Resume pulsing

    else:
        fading_in = True  # Reset this so we fade in next time
        if not fade_started:
            fade_started = True
        ...
        if not has_faded and fade_alpha > 0:
            ...
            text_surface = sub_font.render("Press Enter to Start", True, intro_colors["black"]).convert_alpha()
            text_surface.set_alpha(fade_alpha)
            text_rect = text_surface.get_rect(center=(logo_target_x, center_y + 240))
            screen.blit(text_surface, text_rect)
        elif fade_alpha <= 0:
            has_faded = True
